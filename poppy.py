...
@functools.partial(jax.jit, static_argnums = 1)
def i2v(i,f):
    return jnp.floor_divide((i % f.p) * f.ONE, f.BASIS) % f.p
...
def lift(i,f):  
    if type(i) == int:
        i = jnp.array(i, dtype = DTYPE).reshape((1,1,1))
...
class array:
    ...
    def trace(self):  
        def t(i):
            return jnp.trace(block(self.REP[i,:,:], self.field)) % self.field.p  
        return array(jax.vmap(t)(jnp.arange(self.shape[0])), dtype = self.field, lifted = True)

    def det(self):
        b = block(self.REP, self.field)
        def d(i):
            return qdet(b[i], self.field.INV, self.field.p, 32)
        return array(jax.vmap(d)(jnp.arange(self.shape[0])), dtype = self.field, lifted = True)
    ...
    def inv(self):    
        ...
        return array(jax.vmap(f)(jnp.arange(self.shape[0])), dtype = self.field, lifted = True)
...
def Z2(q):
    Zq = jnp.arange(q)
    Z2q = jnp.array([jnp.tile(Zq,q), jnp.repeat(Zq,q)]).T
    return Z2q # The finite ring Z/q x Z/q.

@functools.partial(jax.jit, static_argnums = 0)
def M2(q):
    Z2q = Z2(q)
    M2q = jnp.array([jnp.tile(Z2q.T,q*q).T, jnp.repeat(Z2q.T,q*q).reshape(2,-1).T]).swapaxes(0,1).reshape(-1,2,2)
